...
class UserGoals(Database):
  ...
  def __init__(self, title=None, description=None, due_date=None, achieved=0, expired=0):
      super().__init__()
      self.title = title
  ...
  def add_goal(self) -> dict:
    ...
    if self.current_user:
      try:
        self.cursor.execute(
            '''
            INSERT INTO user_goals (title, description, user_id, due_date, achieved, expired) 
            VALUES (?, ?, ?, ?, ?, ?)
            ''',
            (self.title, self.description, self.current_user.id, self.due_date, self.achieved, self.expired)
        )
        self.commit()
  ...
  def get_all_goals(self) -> dict:
    ...
    if self.current_user:
      try:
        ...
        goals_list = []
        for g in goals:
          goal = UserGoals(g[1], g[2], g[3], g[4], g[5])
          goal.id = g[0]
    ...
    if self.current_user and self.id:
      try:
        self.cursor.execute(
            '''
            UPDATE user_goals
            SET title = ?, description = ?, due_date = ?, achieved = ?, expired = ?
            WHERE id = ? AND user_id = ?
            ''',
            (self.title, self.description, self.due_date, self.achieved, self.expired, self.id, self.current_user.id)
        )
        self.commit()
        return {"successful": True}
      except Exception as e:
